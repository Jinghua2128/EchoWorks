...
- 角色说话姿势由语音开始和结束事件控制，并在语音结束、出错、静音、切换场景或离开页面时恢复静止。语音开始时立即显示已确认的说话图片，播放期间每 170ms 与静止图片交替，并使用克制的 3px 说话浮动；减少动态效果模式只显示静态说话图片。
- 屏幕宽度超过 920px 时，两名角色会使用 72-140px 响应式内缩，让笔记本和桌面视图中的对话距离更自然；平板与手机位置不变。
- 情景脚本与视觉 CSS 缓存版本号为 `20260729-speaking-motion`。所有语义姿势状态仍只会使用该角色已确认的静止/说话图片。
